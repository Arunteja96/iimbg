...
ggplot(df,aes(x=cyl,y=gear,fill=cyl)) + geom_bar(stat = 'identity',position = position_dodge())


df %>% group_by(cyl,gear) %>% summarise(CNT= n())
ggplot(df %>% group_by(cyl,gear) %>% summarise(CNT= n()),aes(x=cyl,y=CNT,fill=gear)) + geom_bar(stat='identity',position = position_stack()
)
